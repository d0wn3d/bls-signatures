...
template <typename BLSType>
BLSType LagrangeInterpolate(const std::vector<BLSType> &vec,
                            const std::vector<Bytes> &ids);
} // end namespace Poly
...
struct PolyOpsBase {
  ...
  void ModOrder(bn_t &r) { bn_mod(r, r, order); }
};

template <typename G> struct PolyOps;

template <> struct PolyOps<PrivateKey> : PolyOpsBase {
  PrivateKey Add(const PrivateKey &a, const PrivateKey &b) {
    return PrivateKey::Aggregate({a, b});
  }

  PrivateKey Mul(const PrivateKey &a, const bn_t &b) { return a * b; }
};

template <> struct PolyOps<G1Element> : PolyOpsBase {
  G1Element Add(const G1Element &a, const G1Element &b) {
    return pThresholdScheme->Aggregate({a, b});
  }

  G1Element Mul(const G1Element &a, const bn_t &b) { return a * b; }
};

template <> struct PolyOps<G2Element> : PolyOpsBase {
  G2Element Add(const G2Element &a, const G2Element &b) {
    return pThresholdScheme->Aggregate({a, b});
  }

  G2Element Mul(const G2Element &a, bn_t &b) { return a * b; }
};
...
template <typename BLSType>
BLSType Poly::LagrangeInterpolate(const std::vector<BLSType> &vec,
                                  const std::vector<Bytes> &ids) {
  typedef PolyOps<BLSType> Ops;
  Ops ops;
...
PrivateKey Threshold::PrivateKeyShare(const std::vector<PrivateKey> &sks,
                                      const Bytes &id) {
  return Poly::Evaluate(sks, id);
}

PrivateKey Threshold::PrivateKeyRecover(const std::vector<PrivateKey> &sks,
                                        const std::vector<Bytes> &ids) {
  return Poly::LagrangeInterpolate(sks, ids);
}

G1Element Threshold::PublicKeyShare(const std::vector<G1Element> &pks,
                                    const Bytes &id) {
  return Poly::Evaluate(pks, id);
}

G1Element Threshold::PublicKeyRecover(const std::vector<G1Element> &sks,
                                      const std::vector<Bytes> &ids) {
  return Poly::LagrangeInterpolate(sks, ids);
}

G2Element Threshold::SignatureShare(const std::vector<G2Element> &sigs,
                                    const Bytes &id) {
  return Poly::Evaluate(sigs, id);
}

G2Element Threshold::SignatureRecover(const std::vector<G2Element> &sigs,
                                      const std::vector<Bytes> &ids) {
  return Poly::LagrangeInterpolate(sigs, ids);
}

G2Element Threshold::Sign(const PrivateKey &privateKey,
                          const Bytes &vecMessage) {
  return pThresholdScheme->Sign(privateKey, vecMessage);
}

bool Threshold::Verify(const G1Element &pubKey, const Bytes &vecMessage,
